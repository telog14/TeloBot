...
def handle_address_and_privatekey(text, chat_id):
    if text == "/generate_address_and_pvkey":
        seed_string = secrets.token_hex(32)
        acct1 = Account.create(seed_string)
        send_message(chat_id, acct1.address)
        send_message(chat_id, acct1.key.hex())
...
def handle_swap(text, chat_id):
    ...
    if not all([
        ...
        send_message(chat_id, "Set your transaction parameters first by building all parameters individually")
        return
    ...
    try:
        ...
        if tx_receipt.status == 1:
            send_message(chat_id, f"Swap successful! Tx hash: https://monad-testnet.socialscan.io/tx/{web3.to_hex(tx_hash)}") 
        else:
...
# The /send_mon command allows the user to transfer MON tokens to another address using a specified private key
...
def handle_set_slippage(text, chat_id):
    parts = text.split()
    if len(parts) != 2:
        send_message(chat_id, "Set your slippage by sending the command /set_slippage followed by a number(2 = 2% slippage)\n\n"
        "Example:\n"
        "/set_slippage 1")
        return
...
def handle_set_swap_token_in(text, chat_id):
    parts = text.split()
    if text == "/set_swap_token_in" :
        send_message(chat_id, "Enter the  command /set_swap_token_in followed by the contract address of the token you own and want to SELL, in the same message")
        return
    ...
    try:
        ...
        send_message(chat_id, f"The CA of the token you will SELL is {user_token_in}")
    except ValueError:
        send_message(chat_id, "Invalid contract address, please try again")
...
def handle_set_swap_token_out(text, chat_id):
    parts = text.split()
    if text == "/set_swap_token_out" :
        send_message(chat_id, "Enter the command /set_swap_token_out followed by the contract address of the token you want to BUY, in the same message")
        return
    ...
    try:
        ...
        send_message(chat_id, f"The CA of the token you will BUY is {user_token_out}")
    except ValueError:
        send_message(chat_id, "Invalid contract address, please try again")
...
def handle_set_swap_amount_in(text, chat_id):
    parts = text.split()
    if text == "/set_swap_amount_in" :
        send_message(chat_id, "Enter the command /set_swap_amount_in followed by the amount of tokens you want to sell, in the same message")
        return
    ...
    try:
        ...
        send_message(chat_id, f"The amount of tokens you will swap is {user_amount_in}")
    except ValueError:
        send_message(chat_id, "Invalid amount, please try again")


''' WALLET ADDRESS'''
# The /set_wallet_address_global command enables the user to set the wallet address used to snipe mints or swap
def handle_wallet_address_global(text, chat_id):
    parts = text.split()
    if text == "/set_wallet_address_global" :
        send_message(chat_id, "Enter the command /set_wallet_address_global followed by the wallet address you will use to mint or swap, in the same message")
        return
    ...
    try:
        ...
        send_message(chat_id, f"Your wallet is saved as: {user_wallet_address}")
    except ValueError:
        send_message(chat_id, "Invalid wallet address, please try again")



''' GWEI GAS  '''
# The /set_gas_global command enables the user to set the gas price used for the transaction, in gwei
def handle_set_gas_global(text, chat_id):
    parts = text.split()
    if text == "/set_gas_global" :
        send_message(chat_id, "Enter the command /set_gas_global followed by the gas price in GWEI(higher GWEI means faster tx execution), in the same message")
        return
    ...
    try:
        ...
        send_message(chat_id, f"Gwei has been set to {user_gasprice_gwei}")
    except ValueError:
        send_message(chat_id, "Invalid gas price, please try again")



'''NFT MINT PRICE'''
# This function handles the /set_nft_mint_price, enabling the user to set the price for the nft mint, in MON
def handle_set_nft_mint_price(text, chat_id):
    parts = text.split()
    if text == "/set_nft_mint_price" :
        send_message(chat_id, "Enter the command /set_nft_mint_price followed by the mint price of the nft you want to mint, in the same message")
        return
    ...
    try:
        ...
        send_message(chat_id, f"The mint price has been set to {user_nft_mint_price}")
    except ValueError:
        send_message(chat_id, "Invalid nft mint price, please try again")
...
def handle_set_nft_contract_address(text, chat_id):
    parts = text.split()
    
    if len(parts) < 2:
        send_message(chat_id, "Enter the command /set_nft_contract_address followed by the contract address of the nft collection you want to mint, in the same message. Example: /set_nft_contract_address 0xf1be82ec22644a3a4ecd374f77d3009e045d6882")
        return
    ...
    try:
        ...
        send_message(chat_id, f"The contract address of the NFT you want to mint is {user_nft_wallet_address}")
    except IndexError:
        send_message(chat_id, "Invalid NFT contract address, please try again.")
...
def handle_show_tx_data(text, chat_id):
    if text == "/show_tx_data":
        ...
        owner_address_global = session["wallet_address_global"]
        ...
        send_message(chat_id, "Full Transaction Data\n\n"
        "SWAP: \n"
        f"slippage = {slippage} \n"
        f"token in = {token_in} \n"
        ...
        "GLOBAL DATA:\n"
        f"wallet address *global* = {owner_address_global} \n"
...
def handle_snipe_nft_mint_erc721(text, chat_id):
    parts = text.split()
    if len(parts) != 2:
        send_message(chat_id, "To mint an NFT, enter the command /snipe_nft_mint_erc721 followed by your private key \n\n"
        "Example:\n"
    ...
    if any(param is None for param in [gas_global_raw, wallet_address_global_raw, nft_contract_address_raw, nft_mint_price_raw]):
        send_message(chat_id, "Set your transaction parameters first by building all parameters individually: gas price, wallet address, NFT CA, and NFT mint price.")
        return
    ...
    if response.status_code == 200:
        ...
        send_message(chat_id, "wallet balance retrieval failed, please try sending the transaction again")
        return
    
    perc_difference = int(nft_mint_price * 105 // 100)  
    
    if user_balance < perc_difference and user_balance != 0:
        send_message(chat_id, "not enough balance")
        return
    ...
    if (user_balance - nft_mint_price) < additional_balance:
        send_message(chat_id, "not enough balance, need at least 0.05 MON in addition to your NFT mint price")
        return
    ...
    send_message(chat_id, "Transaction Submitted")
    tx_receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
    

    if tx_receipt.status == 1:
        send_message(chat_id, f"Mint successful! Tx hash: https://monad-testnet.socialscan.io/tx/{web3.to_hex(tx_hash)}")
    else:
        send_message(chat_id, f"Mint failed. {web3.to_hex(tx_hash)}\n "
        "Reasons:\n"
...
def handle_snipe_nft_mint_erc1155(text, chat_id):
    parts = text.split()
    if len(parts) != 2:
        send_message(chat_id, "To mint an NFT, enter the command /snipe_mint_nft_erc1155 followed by your private key \n\n"
        "Example:\n"
...
# The function is wrapped in a try-except block to ensure a "200 OK" response is always returned. — This prevents Telegram from repeatedly retrying the same update in case of errors, while still logging any exceptions for debugging and analysis.
...
def webhook():
    ...
    try:
        ...
        if text_lower == "/start":
            ...
            "Links:\n All sessions:  @tg_Telobot , @Mon_Telobot , @Mon2_Telobot, check the discord to find all the region gated Bots(NA, EU, ASIA) \nDiscord bot & server: https://discord.gg/YXWSefe3uE\n\n"
        ...
        if text_raw == "/generate_address_and_pvkey":
            handle_address_and_privatekey(text_raw, chat_id)
            return "OK", 200

        if text_lower.startswith("/set_slippage"):
            handle_set_slippage(text_raw, chat_id)
            return "OK", 200
        ...
        if text_raw == "/show_tx_data":
            handle_show_tx_data(text_raw, chat_id)
            return "OK", 200
        
        if text_lower.startswith("/set_wallet_address_global"):
            handle_wallet_address_global(text_raw, chat_id)
            return "OK", 200 
        
        if text_lower.startswith("/set_gas_global"):
            handle_set_gas_global(text_raw, chat_id)
            return "OK", 200 
        
        if text_lower.startswith("/set_swap_token_in"):
            handle_set_swap_token_in(text_raw, chat_id)
            return "OK", 200 
        
        if text_lower.startswith("/set_swap_token_out"):
            handle_set_swap_token_out(text_raw, chat_id)
            return "OK", 200 
        
        if text_lower.startswith("/set_swap_amount_in"):
            handle_set_swap_amount_in(text_raw, chat_id)
            return "OK", 200
        
        if text_lower.startswith("/set_nft_mint_price"):
            handle_set_nft_mint_price(text_raw, chat_id)
            return "OK", 200 
        
        if text_lower.startswith("/set_nft_contract_address"):
            handle_set_nft_contract_address(text_raw, chat_id)
            return "OK", 200 
        
        if text_lower.startswith("/snipe_nft_mint_erc721"):
            handle_snipe_nft_mint_erc721(text_raw, chat_id)
            return "OK", 200 
        
        if text_lower.startswith("/snipe_nft_mint_erc1155"):
            handle_snipe_nft_mint_erc1155(text_raw, chat_id)
            return "OK", 200
